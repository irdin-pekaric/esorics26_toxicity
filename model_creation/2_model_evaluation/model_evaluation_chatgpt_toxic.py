...
DEFAULT_PROMPT = (
    ...
    "Classify the following message as either 'toxic' or 'nontoxic'.\n"
    "The message may contain slang, sarcasm, abbreviations, or profanity.\n"
    "Your response must be only: toxic OR nontoxic.\n\n"
    "Message:\n\n{text}\n\n"
    "Example response: toxic\n"
)
DEFAULT_OUT_ROOT = Path(__file__).resolve().parent / "evaluations_chatgpt_toxic"
...
def parse_toxicity_from_text(text: Optional[str]) -> Optional[int]:
    ...
    s = re.sub(r'\bnon\s*[-_]?\s*toxic\b', 'nontoxic', s)
    s = re.sub(r'(?<=\w)[-_](?=\w)', '', s)
    s = re.sub(r'[^a-z\s]', ' ', s)
    s = re.sub(r'\s+', ' ', s).strip()
    if not s:
        return None
    m = re.search(r"\b(nontoxic|toxic)\b", s)
    if not m:
        return None
    token = m.group(1)
    if token == 'toxic':
